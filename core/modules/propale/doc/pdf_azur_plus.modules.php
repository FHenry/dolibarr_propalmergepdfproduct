...
/**
 * \file /propalmergepdfproduct/core/modules/propale/doc/pdf_azur_plus.modules.php
 ...
 */
class pdf_azur_plus extends ModelePDFPropales {
	var $db;
	var $name;
	var $description;
	var $type;
	...
	function __construct($db) {
		...
		$this->db = $db;
		$this->name = "azur_plus";
		$this->description = $langs->trans ( 'PropalMergePdfProductDocModelAzurDescription' );
	 ...
	 * @param string $srctemplatepath of source filename for generator using a template file
	 * @param int $hidedetails show line details
	 * @param int $hidedesc show desc
	 * @param int $hideref show ref
			...
			if (file_exists ( $dir )) {
				...
				for($i = 0; $i < $nblignes; $i ++) {
					...
					if ($pageposafter > $pageposbefore) 					// There is a pagebreak
					{
						...
						$pageposafter = $pdf->getPage ();
						$posyafter = $pdf->GetY ();
						if ($posyafter > ($this->page_hauteur - ($heightforfooter + $heightforfreetext + $heightforinfotot))) 						// There is no space left for
						                                                                                                  // total+free text
						{
				...
				*/
				...
				$parameters = array (
					'file' => $file,'object' => $object,'outputlangs' => $outputlangs 
				);
				global $action;
	 ...
	 * @param int $posy y in PDF
	 * @param Translate $outputlangs langs for output
	 ...
	 * @param int $deja_regle deja regle
	 * @param int $posy
	 * @param Translate $outputlangs
	 * @return int pour suite
	 */
		...
		if (empty ( $conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT )) {
			...
			if (! empty ( $conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_ISNULL ) && $tvaisnull) {
				...
				foreach ( $this->localtax1 as $localtax_type => $localtax_rate ) {
					if (in_array ( ( string ) $localtax_type, array (
						'1','3','5','7' 
					) ))
						continue;
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0) {
		...
		// Output Rect
		$this->printRect ( $pdf, $this->marge_gauche, $tab_top, $this->page_largeur - $this->marge_gauche - $this->marge_droite, $tab_height, $hidetop, $hidebottom ); // Rect
		                                                                                                                                                       // prend
		                                                                                                                                                       // une
		                                                                                                                                                       // longueur
		                                                                                                                                                       // en
		...
		if (empty ( $hidetop )) {
			$pdf->line ( $this->marge_gauche, $tab_top + 5, $this->page_largeur - $this->marge_droite, $tab_top + 5 ); // line prend une position y en 2eme param
			                                                                                                  // et 4eme param
